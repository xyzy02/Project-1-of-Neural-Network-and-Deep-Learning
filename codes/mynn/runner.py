...
class RunnerM():
    ...
    def _set_model_training(self, training):
    ...
    def train(self, train_set, dev_set, **kwargs):
        ...
        max_iterations_per_epoch = kwargs.get("max_iterations_per_epoch", None)
        ...
        for epoch in range(num_epochs):
            ...
            batches_per_epoch = int(X.shape[0] / self.batch_size) + 1
            if max_iterations_per_epoch is not None:
                batches_per_epoch = min(batches_per_epoch, int(max_iterations_per_epoch))
            ...
            print(f"epoch: {epoch} [Dev] loss: {dev_loss}, score: {dev_score}")
    ...
    def evaluate(self, data_set):
        X, y = data_set
        self._set_model_training(False)
        try:
            logits = self.model(X)
